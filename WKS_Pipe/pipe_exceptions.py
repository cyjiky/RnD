class ConfigException(Exception):
    """
    Gets raised if invalid config provided.
    E.g. : missing keys, values
    """
...
class Exceptions:
    ...
    def validate_pipe_dimensions(
        ...
        if longest_dim != None and longest_dim <= 0:
            raise ValueError(
                "The maximum diameter exists only in the range (0; +infty)"
            )

        if shortest_dim != None and shortest_dim <= 0:
            raise ValueError(
                "The minimum diameter can only exist in the range (0; +infty)"
            )
        ...
        if longest_dim != None and shortest_dim != None and longest_dim <= shortest_dim:
            raise ValueError(
                "The minimum diameter must be strictly less than the maximum"
            )
